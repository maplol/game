...
export interface SceneManagerContextValue {
  ...
  setSceneState: (key: string | symbol, value: any) => void;
  getSceneState: (key: string | symbol) => any;
}

export const SceneManagerContext = React.createContext<
  SceneManagerContextValue
>(null!);
  ...
  const api = useMemo<SceneManagerContextValue>(
    () => ({
      ...
      setSceneState(key, value) {
        ...
        sceneStore.current.set(`${currentScene}.${keyString}`, value);
      },
      getSceneState(key) {
        const keyString = key as string;
        return sceneStore.current.get(`${currentScene}.${keyString}`);
      },
    }),
